  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: meeting } = await supabase
        .from('meetings')
        .select('id, instructor_id, center_id, meet_date, end_date, max_participants, description')
        .eq('id', id)
        .single()

      if (!meeting || meeting.instructor_id !== user.id) {
        router.push('/')
        return
      }
      ...
      <main className="max-w-2xl mx-auto px-4 py-6">
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm p-6 space-y-5">
          ...
          <button
            ...
            className="w-full py-2.5 rounded-lg text-sm font-medium text-red-500 hover:bg-red-50 border border-red-200"
          >
            모임 삭제
          </button>
        </form>
